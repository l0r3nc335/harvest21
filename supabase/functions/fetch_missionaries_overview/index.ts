...
serve(async (req)=>{
  try {
    ...
    const missionaryUserIds = (rawMissionaries || [])
      .map((m) => m.user_id)
    ...
    const activeMissionaries = (rawMissionaries || []).filter(
      (m) => m.user_id && activeUserIds.has(m.user_id)
    );

    console.log(`📊 Missionaries after Active-user filter: ${activeMissionaries.length}`);
    ...
    if (userId) {
      ...
      if (currentUserMissionaryId) {
        const { data: missionaryFollows } = await supabaseAdmin
          .from("missionary_missionary_followers")
          .select("followed_missionary_id, status")
          .eq("follower_missionary_id", currentUserMissionaryId);
        
        missionaryFollows?.forEach(f => {
          const existing = followerStatusMap.get(f.followed_missionary_id);
          if (!existing || existing === "none") {
            followerStatusMap.set(f.followed_missionary_id, f.status);
          }
        });
      }
    }
    ...
    console.log(`\n📊 GROUPING SUMMARY:`);
    console.log(`   Total fetched: ${rawMissionaries?.length || 0} (${(rawMissionaries?.length || 0) - activeMissionaries.length} non-Active-user filtered out)`);

...
import { createChurch } from "@/app/admin/churches/actions";
...
type CreateChurchFormProps = {
  isOpen: boolean;
  onClose: () => void;
  onSubmit?: (result: { church: { id: number; name: string; city?: string | null; country?: string | null }; user: { status?: string; last_activity?: string | null } }) => void | Promise<void>;
};

export function CreateChurchForm({ isOpen, onClose, onSubmit }: CreateChurchFormProps) {
  ...
  const handleFormSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (result.success) {
        reset();
        onClose();
        toast.success(result.message || "Church created successfully!");
        
        // Call the onSubmit callback if provided (pass the created church data)
        if (onSubmit && result.church) {
          await onSubmit({ church: result.church, user: result.user ?? undefined });
        } else if (!onSubmit) {
  ...
  return (
    <SlidePanel isOpen={isOpen} onClose={onClose} title="Create New Church">
